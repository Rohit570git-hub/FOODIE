...
public class PlaceYourOrderAdapter extends RecyclerView.Adapter<PlaceYourOrderAdapter.MyViewHolder> {
    ...
    public void onBindViewHolder(@NonNull PlaceYourOrderAdapter.MyViewHolder holder, int position) {
        ...
        holder.menuPrice.setText("$"+String.format("%.2f", menuList.get(position).getPrice()*menuList.get(position).getTotalInCart()));
        holder.menuQty.setText("×" + menuList.get(position).getTotalInCart());
        int foodtype=menuList.get(position).getFoodtype();
        if(foodtype==1){
            holder.thumbImage.setImageResource(R.drawable.nonvegi);
        }else {
            holder.thumbImage.setImageResource(R.drawable.vegicon);
        }
